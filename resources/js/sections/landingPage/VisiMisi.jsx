...
const dataMobile =[
    slide1mobile,slide2mobile,slide3mobile
]
  ...
  return (
    <section className="bg-gradient-to-b mt-20 from-[#F0E5DB] py-10 lg:py-40  from-0% relative  flex justify-center items-center via-white via-50% to-[#F0E5DB] to-100%">
      ...
      <div className="container  lg:px-16 md:px-10 px-4 ">
        ...
        }} aria-label="My Favorite Images"  className={'lg:block hidden'} >
          
            {
                data.map((item,i)=>(
          <SplideSlide key={i}>
            <img src={item} draggable='false' className="w-4/5 mx-auto aspect-auto" alt={`Image ${i}`} />
          </SplideSlide>
        ...
        }} aria-label="My Favorite Images"  className={'block lg:hidden'} >
      ...
      <img src={hiasan} alt="hiasan-img" draggable='false' className="absolute bottom-0 z-20 translate-y-1/2" />
    </section>
  );
};
